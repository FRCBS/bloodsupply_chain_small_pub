...
glimpse(sim_data) #generates the view of data frame below 
```
...
The aim of this project is to identify which variables best predict the future five-year donation activity of blood donors enrolled in the FinDonor research project. A directed acyclic graph (DAG) was used to map out which predictor variables could have meaningful effects on the outcome variable and how the models would need to be adjusted to account for biasing effects. While in reality there are more complex and bidirectional relationships, for the sake of modelling, only the ones thought to be most influential were chosen to be displayed here. The dataset contains 28 variables, of which 17 were included in the final models. Ferritin levels and previous two-year donation count were chosen as the exposure variables

```{r}
knitr::include_graphics("../data/DAGnoexp.png")
```
...
```{r}
knitr::include_graphics("../data/DAGexpDonationC.png")
```
...
p2 <- ggplot(effects_df, aes(x = term, y = estimate, ymin = conf.low, ymax = conf.high, color = model)) +
 geom_pointrange(aes(shape = point_shape), position = position_dodge(width = 0.6), size = 0.8) +
...
p3 <- ggplot(effects_df, aes(x = term, y = estimate, ymin = conf.low, ymax = conf.high, color = model)) +
 geom_pointrange(aes(shape = point_shape), position = position_dodge(width = 0.6), size = 0.8) +
...
p4 <- ggplot(data = mindata_clean, aes(x = Ferritin, y = FUT_Whole_Blood_Count, color = Group)) +
...
p5 <- ggplot(data = mindata_clean, aes(x = Age, y = FUT_Whole_Blood_Count, color = Group)) +
   geom_point(alpha = 0.4) +
  geom_smooth(method = "lm", se = FALSE) +
   facet_grid(Group ~ ., scales = "free_y")  +
   labs(title = "Fig 2.",
...
p6<- ggplot(data = mindata_clean, aes(x = DaysToPreviousFB, y = FUT_Whole_Blood_Count, color = Group)) +
   geom_point(alpha = 0.4) +
 ...
 p7 <- ggplot(data = mindata_clean, aes(x = BMI, y = FUT_Whole_Blood_Count, color = Group)) +
   geom_point(alpha = 0.4) +
  geom_smooth(method = "lm", se = FALSE) +
   facet_grid(Group ~ ., scales = "free_y")  +
   labs(title = "Fig 4.",
...
p8 <- ggplot(data = mindata_clean, aes(x = TwoYearsFromStartCount_FB, y = FUT_Whole_Blood_Count, color = Group)) +
   geom_point(alpha = 0.4) +
  geom_smooth(method = "lm", se = FALSE) +
   facet_grid(Group ~ ., scales = "free_y")  +
   labs(title = "",
...
```{r corrmatrix, fig.height=7, fig.width=7}
...
col <- colorRampPalette(c("#3028c9", "white", "#d4289d"))(200)
...
# Save the plot as a PNG file with a custom name
ggsave(paste0(folder_path, "effect_estimates.png"), plot = p1, width = 8, height = 6, dpi = 300)

# Save the plot as a PDF file with a custom name
ggsave(paste0(folder_path, "effect_estimates.pdf"), plot = p1, width = 8, height = 6)
...
```{r}
ggsave(paste0(folder_path, "effect_estimates_strat1.png"), plot = p2, width = 8, height = 6, dpi = 300)
ggsave(paste0(folder_path, "effect_estimates_strat1.pdf"), plot = p2, width = 8, height = 6)
```

```{r}
ggsave(paste0(folder_path, "effect_estimates_strat2.png"), plot = p3, width = 8, height = 6, dpi = 300)
ggsave(paste0(folder_path, "effect_estimates_strat2.pdf"), plot = p3, width = 8, height = 6)

```

```{r}
ggsave(paste0(folder_path, "scatter_ferritin.png"), plot = p4, width = 8, height = 6, dpi = 300)
ggsave(paste0(folder_path, "scatter_ferritin.pdf"), plot = p4, width = 8, height = 6)
```

```{r}
ggsave(paste0(folder_path, "scatter_age.png"), plot = p5, width = 8, height = 6, dpi = 300)
ggsave(paste0(folder_path, "scatter_age.pdf"), plot = p5, width = 8, height = 6)
```

```{r}
ggsave(paste0(folder_path, "scatter_daystoFB.png"), plot = p6, width = 8, height = 6, dpi = 300)
ggsave(paste0(folder_path, "scatter_daystoFB.pdf"), plot = p6, width = 8, height = 6)
```

```{r}
ggsave(paste0(folder_path, "scatter_BMI.png"), plot = p7, width = 8, height = 6, dpi = 300)
ggsave(paste0(folder_path, "scatter_BMI.pdf"), plot = p7, width = 8, height = 6)

```

```{r}
ggsave(paste0(folder_path, "scatter_doncount.png"), plot = p8, width = 8, height = 6, dpi = 300)
ggsave(paste0(folder_path, "scatter_doncount.pdf"), plot = p8, width = 8, height = 6)
```
